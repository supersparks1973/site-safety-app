...
async function startApp() {
  ...
  app.get('/api/training', authenticate, async (req, res) => {
    ...
    if (['admin', 'project_manager', 'external_view'].includes(req.user.role)) {
      ...
      // Operatives: own user_id OR orphan records whose external_name matches the
      // operative's full_name in the DB (subquery — don't rely on JWT freshness).
      const { rows } = await pool.query(
        `SELECT t.*, COALESCE(u.full_name, t.external_name, 'Unknown') as operative_name
          ...
          WHERE t.user_id = $1
             OR (
               t.user_id IS NULL
               AND LOWER(TRIM(t.external_name)) = (SELECT LOWER(TRIM(full_name)) FROM users WHERE id = $1)
             )
